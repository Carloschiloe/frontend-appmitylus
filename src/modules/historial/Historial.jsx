...
export default function Historial() {
  ...
  async function loadAllData(signal) {
    ...
    try {
      ...
      const results = await Promise.all(endpoints.map(url => apiClient.get(url, { signal })));
        ...
        <header className="mx-hero">
          <div className="mx-hero-content">
            ...
            <div style={{ display: 'flex', gap: '16px', marginTop: '12px' }}>
              <span className="mx-badge" style={{ background: 'rgba(255,255,255,0.2)', color: 'white' }}>
                <History size={14} style={{ marginRight: '6px' }} /> {selectedProvider.events.length} Gestiones
              </span>
              <span className="mx-badge" style={{ background: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.8)' }}>
                <Clock size={14} style={{ marginRight: '6px' }} /> Última actividad: {formatDate(selectedProvider.lastActivity)}
              </span>
            </div>
          </div>
          <button className="mx-btn mx-btn-outline" style={{ color: 'white', borderColor: 'rgba(255,255,255,0.3)' }} onClick={() => setSelectedProvider(null)}>
