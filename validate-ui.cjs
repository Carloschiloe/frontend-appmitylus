...
const ROUTES = [
  ...
  { path: '/gestion/bandeja',                   key: '02-bandeja',      eyebrow: 'Operaciones · Resumen',              title: 'Resumen Operativo' },
  { path: '/gestion/agenda',                    key: '03-agenda',       eyebrow: 'Operaciones · Agenda',               title: 'Agenda Operacional' },
  { path: '/gestion/tratos',                    key: '04-tratos',       eyebrow: 'Operaciones · Tratos',               title: 'Tratos Comerciales' },
  { path: '/biomasa/programa',                  key: '05-programa',     eyebrow: 'Operaciones · Programa de Cosecha',  title: 'Programa de Cosecha' },
  { path: '/biomasa/muestreos',                 key: '06-muestreos',    eyebrow: 'Operaciones · Muestreos Técnicos',   title: 'Muestreos Técnicos' },
  { path: '/gestion/proveedores',               key: '07-proveedores',  eyebrow: 'Directorio · Proveedores',           title: 'Directorio de Proveedores' },
  { path: '/historial',                         key: '08-historial',    eyebrow: 'Inteligencia · Historial',           title: 'Historial operativo' },
  { path: '/historial?proveedor=algemarin-spa', key: '09-expediente',   eyebrow: 'Inteligencia · Historial',           title: null },
  ...
  { path: '/centros/sanitario',                 key: '12-sanitario',    eyebrow: 'Inteligencia · Sanitario',           title: 'Estado Sanitario' },
];
...
async function run() {
  ...
  const sidebarChecks = {
    hasTratos:     sidebarLinks.some(l => l === 'Tratos'),
  ...
  const sidebarOk = sidebarChecks.hasTratos && sidebarChecks.hasHistorial && !sidebarChecks.hasNeg && !sidebarChecks.hasHistEquipo;
  ...
  for (const route of ROUTES) {
    ...
    const fabY        = await page.evaluate(() => {
      const fab = Array.from(document.querySelectorAll('button')).find(b => window.getComputedStyle(b).position === 'fixed' && b.textContent.includes('Registrar'));
...
function printReport() {
  ...
  for (const r of results) {
    ...
    if (r.key === 'sidebar') {
      console.log(`\n${icon} SIDEBAR   Tratos=${r.hasTratos}  Negociación=${r.hasNeg}  Historial=${r.hasHistorial}  "H/E"=${r.hasHistEquipo}`);
      continue;
    }
    console.log(`\n${icon} ${r.key}`);
