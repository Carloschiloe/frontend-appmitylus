...
(function (global) {
  ...
  function injectCSS(){
    ...
    var css = ''
    +'.cal-wrap{max-width:1200px;margin:0 auto;padding:16px}'
    +'.cal-card{background:#fff;border:1px solid #e5e7eb;border-radius:16px;padding:18px;box-shadow:0 8px 20px rgba(17,24,39,.06)}'
    +'.cal-title{margin:0;font-weight:800;color:#2b3440;font-size:22px;display:flex;gap:8px;align-items:center}'
    +'.cal-btn{background:#eef2ff;border:1px solid #c7d2fe;color:#1e40af;height:34px;border-radius:8px;padding:0 10px;cursor:pointer;font-weight:700}'
    +'.cal-btn.s{height:28px;padding:0 8px;border-radius:8px;font-size:12px}'
    ...
    +'.seg button{height:32px;padding:0 10px;background:#f3f4f6;border:0;cursor:pointer;font-weight:700}'
    +'.seg button.on{background:#2155ff;color:#fff}'
    +'.cal-tabs{display:flex;gap:6px;flex-wrap:wrap}'
    +'.cal-tab{background:#f8fafc;border:1px solid #e5e7eb;border-radius:999px;padding:6px 10px;cursor:pointer;font-weight:800;color:#374151;font-size:12px}'
    +'.cal-tab.on{background:#2155ff;color:#fff;border-color:#2155ff}'

    /* Cabecera */
    +'.titleRow{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:6px}'
    +'.controlRow{display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin:4px 0 8px}'
    +'.monthgrp{display:inline-flex;align-items:center;gap:6px;background:#f8fafc;border:1px solid #e5e7eb;border-radius:10px;padding:4px 6px}'
    +'.monthlbl{font-weight:800;color:#1e3a8a;padding:0 4px}'
    +'.kpisRow{display:flex;gap:8px;flex-wrap:wrap;margin:6px 0 10px}'

    +'.kpi{background:#f3f4f6;border:1px solid #e5e7eb;border-radius:10px;padding:10px 10px;font-weight:700;min-width:140px}'
    +'.kpi .lbl{font-size:12px;color:#6b7280;font-weight:600}'
    +'.kpi input{height:30px;border:1px solid #e5e7eb;border-radius:8px;padding:0 8px;background:#fff;width:96px;margin-left:6px}'

    +'.cal-small{font-size:12px;color:#6b7280}'

    /* ---- Tarjetas ---- */
    +'.cal-cardrow{margin:6px 0 8px}'
    +'#cardDeck{display:flex;gap:8px;align-items:stretch;width:100%;overflow:hidden}'
    +'#cardDeck .fcard{flex:1 1 0;min-width:0}'
    +'.fcard{position:relative;background:#f8fafc;border:1px solid #e5e7eb;border-radius:12px;padding:10px 10px 10px 14px;display:flex;align-items:center;justify-content:space-between;gap:10px;cursor:pointer;transition:box-shadow .15s,border-color .15s}'
    ...
    +'.fcard .title{font-weight:800;color:#374151;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:13px}'
    +'.fcard .sub{font-size:10px;color:#6b7280}'
    +'.qtyBlk{display:flex;flex-direction:column;align-items:flex-end;gap:4px;min-width:78px;flex:0 0 auto}'
    +'.fcard .qty{font-weight:800;font-size:16px;color:#1f2937;white-space:nowrap}'
    +'.fcard .unit{font-size:12px;color:#6b7280;margin-left:4px}'
    +'.pct{font-size:11px;color:#6b7280}'
    +'.pctbar{width:90px;height:6px;border-radius:999px;background:#e5e7eb;overflow:hidden}'
    +'.pctbar span{display:block;height:100%}'

    /* Calendario */
    +'.cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:8px}'
    +'.cal-dayname{font-weight:800;color:#64748b;text-align:center;padding:6px;border:1px solid #e5e7eb;border-radius:8px;background:#f3f4f6;font-size:12px}'
    +'.cal-cell{background:#fcfdff;border:1px solid #e5e7eb;border-radius:12px;min-height:110px;padding:8px;display:flex;flex-direction:column;gap:6px;position:relative}'
    ...
    +'.badge{margin-left:6px;font-size:10px;padding:2px 6px;border:1px solid #e5e7eb;border-radius:999px;background:#fff}'
    +'.badge.red{background:#fee2e2;border-color:#fecaca;color:#991b1b}'
    +'.pill{display:inline-flex;align-items:center;gap:6px;padding:2px 6px;border-radius:8px;background:#eef2ff;color:#1e40af;border:1px solid #c7d2fe;font-size:10px}'
    ...
    +'.chip .prov{font-weight:600;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:10px;max-width:140px}'
    +'.chip .qty{font-weight:700;white-space:nowrap;font-size:10px}'
    ...
    +'.modal{width:min(980px,96vw);background:#fff;border:1px solid #e5e7eb;border-radius:14px;box-shadow:0 24px 50px rgba(0,0,0,.2);padding:16px}'
