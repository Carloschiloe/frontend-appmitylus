...
function injectCSS(){
  ...
  var css = ''
  /* ====== LAYOUT + SIDEBAR (como tus otras vistas) ====== */
  + 'body{margin:0;background:#f6f8ff}'
  + '.mmpp-layout{display:grid;grid-template-columns:260px 1fr;gap:18px;align-items:start;max-width:1400px;margin:0 auto;padding:18px}'
  + '.mmpp-main{min-width:0}'
  + '.mmpp-nav{position:sticky;top:18px;height:calc(100vh - 36px);background:#0f172a;border:1px solid #111827;border-radius:20px;padding:16px;box-shadow:0 10px 30px rgba(17,24,39,.25);color:#e2e8f0}'
  + '.mmpp-nav .brand{display:flex;align-items:center;gap:10px;margin-bottom:12px;font-weight:900}'
  + '.mmpp-nav .logo{width:32px;height:32px;border-radius:999px;background:#6d28d9;color:#fff;display:grid;place-items:center;font-weight:900}'
  + '.mmpp-nav .section{margin:10px 0 6px;color:#94a3b8;font-size:12px;font-weight:800;letter-spacing:.02em}'
  + '.mmpp-nav a{display:flex;gap:8px;align-items:center;padding:10px 12px;border-radius:12px;text-decoration:none;color:#cbd5e1;font-weight:700}'
  + '.mmpp-nav a .i{width:22px;text-align:center}'
  + '.mmpp-nav a:hover{background:#1f2937}'
  + '.mmpp-nav a.is-active{background:#1e293b;border:1px solid #334155;color:#fff}'
  ...
  + '.pl-filters{display:grid;grid-template-columns:repeat(4,minmax(220px,1fr));gap:10px;align-items:center}'
  ...
  + '.pl-chart-canvas{display:block;width:980px !important;height:360px !important}'
  + '.pl-chart-scroll{overflow-x:auto}'
        ...
        +'<div class="pl-chart-wrap">'
          +'<div class="pl-chart-scroll">'
            +'<div class="pl-chart-frame">'
              +'<canvas id="plChart" class="pl-chart-canvas" width="980" height="360"></canvas>'
            +'</div>'
          +'</div>'
  ...
  function buildDeriv(dispon, asig){
    // suma asignado por disponibilidadId
    var byDisp = {};
    ...
    // deriva registros por disponibilidad
  ...
  function renderChart(rows, axisMode){
    ...
    chartRef.options.plugins.stackTotals = { enabled:true, fontSize:12 };
    chartRef.update();
  }
  ...
  function monthsToUse(){
    ...
    var base = STATE.hideEmpty ? mesesConDatosDispon(STATE.deriv, STATE.filters) : range12();
  ...
  function refresh(){
    ...
    // KPIs (si hay meses seleccionados, ya vienen filtrados)
    renderKPIs(rows);
    // Chart + tabla
