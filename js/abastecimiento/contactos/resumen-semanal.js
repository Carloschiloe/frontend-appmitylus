...
(function injectResumenStyles(){
  ...
  s.textContent = `
    ...
    .col-12{ grid-column: span 12; } @media (min-width:600px){ .col-6{ grid-column: span 6; } }
...
function renderTop(agg){
  ...
  const items = agg.topProveedores.length
    ? agg.topProveedores.map(p => `<li class="collection-item">${p.name}<span class="secondary-content">${p.count}</span></li>`).join('')
    : '<li class="collection-item">—</li>';
  el.innerHTML = `<h6 style="margin-top:12px">Top proveedores por nº de visitas</h6><ul class="collection">${items}</ul>`;
}

function ensureHeadHasFechaProx(tbody){
