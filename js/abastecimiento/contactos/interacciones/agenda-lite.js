...
export function mountAgendaLite(rootEl, items = []) {
  ...
  rootEl.innerHTML = `
    <div class="ag-scope">
      <div class="ag-card int-modern-card ag-modern-card">
        <div class="ag-header">
        ...
        <!-- Filtros -->
        <div class="ag-filters int-modern-filters">
          <div class="ag-filter int-f-item int-f-sm">
          ...
          <div class="ag-filter ag-filter-grow int-f-item int-f-grow">
          ...
          <div class="ag-filter ag-filter-min int-f-item int-f-actions">
            <label>&nbsp;</label>
        ...
        <div class="ag-legend">
          <span><i class="ag-dot ag-dot-muestra"></i> Muestra</span>
          <span><i class="ag-dot ag-dot-visita"></i> Visita</span>
          <span><i class="ag-dot ag-dot-reunion"></i> Reunion</span>
          <span><i class="ag-dot ag-dot-contacto"></i> Contacto</span>
          <span><i class="ag-dot ag-dot-negociacion"></i> Negociacion</span>
        </div>
  ...
  rootEl.querySelector('#agToday').addEventListener('click', () => { view = new Date(); render(true); });
  ...
  function render(refreshCombos = false) {
    ...
    const wd = ['LUN','MAR','MIE','JUE','VIE','SAB','DOM'];
    ...
    // Poblar selects del mes (unicos ordenados)
    if (refreshCombos) {
...
function colorForPaso(paso) {
  ...
  if (key.includes('muestra')) return 'ag-dot-muestra';
  if (key.includes('visita')) return 'ag-dot-visita';
  if (key.includes('reun')) return 'ag-dot-reunion';
  if (key.includes('contacto')) return 'ag-dot-contacto';
  if (key.includes('negoci')) return 'ag-dot-negociacion';
  return 'ag-dot-default';
}
function pillForEstado(estado) {
  const s = (estado || '').toLowerCase();
  if (!s) return '';
  const map = {
    pendiente: 'ag-pill-pendiente',
    agendado: 'ag-pill-agendado',
    hecho: 'ag-pill-hecho',
    completado: 'ag-pill-completado',
    cancelado: 'ag-pill-cancelado',
  };
  return `<span class="ag-pill ${map[s] || 'ag-pill-default'}">${escapeHtml(s)}</span>`;
}
...
function itemCard(it) {
  const dotClass = colorForPaso(it.paso);
  ...
  return `
    <div class="ag-day ${isToday ? 'is-today':''} ${isSunday ? 'is-sunday':''}" data-expanded="0">
      ...
      <div class="ag-day-body">
        ...
        ${moreN ? `<button class="ag-more" data-more-label="+${moreN} mas">+${moreN} mas</button>` : ''}
