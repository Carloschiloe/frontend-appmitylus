...
export async function createCentro(data) {
  return USE_API
    ? await apiCreateCentro(data)
    : await localCreateCentro(data);
}

export async function updateCentro(id, data) {
  return USE_API
    ? await apiUpdateCentro(id, data)
    : await localUpdateCentro(id, data);
}

export async function deleteCentro(id) {
  return USE_API
    ? await apiDeleteCentro(id)
    : await localDeleteCentro(id);
}

/* --------- CRUD Líneas --------- */
export async function addLinea(centroId, data) {
  return USE_API
    ? await apiAddLinea(centroId, data)
    : await localAddLinea(centroId, data);
}

export async function updateLinea(centroId, lineaId, data) {
  return USE_API
    ? await apiUpdateLinea(centroId, lineaId, data)
    : await localUpdateLinea(centroId, lineaId, data);
}

export async function deleteLinea(centroId, lineaId) {
  return USE_API
    ? await apiDeleteLinea(centroId, lineaId)
    : await localDeleteLinea(centroId, lineaId);
}

/* --------- Inventario Línea --------- */
export async function addInventarioLinea(centroId, lineaId, data) {
  return USE_API
    ? await apiAddInventarioLinea(centroId, lineaId, data)
    : await localAddInventarioLinea(centroId, lineaId, data);
}
